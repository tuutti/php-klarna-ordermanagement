...
class RefundsApi
{
     ...
     *
     * @param  string $order_id Order id (required)
     * @param  \Klarna\OrderManagement\Model\RefundObject $refund_object refund_object (required)
     ...
     * @throws \Klarna\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return string|\Klarna\OrderManagement\Model\RefundNotAllowedErrorMessage|\Klarna\OrderManagement\Model\NotFoundErrorMessage
     */
    public function refundOrder($order_id, $refund_object, $klarna_idempotency_key = null, string $contentType = self::contentTypes['refundOrder'][0])
    {
        list($response) = $this->refundOrderWithHttpInfo($order_id, $refund_object, $klarna_idempotency_key, $contentType);
        return $response;
    }
     ...
     *
     * @param  string $order_id Order id (required)
     * @param  \Klarna\OrderManagement\Model\RefundObject $refund_object (required)
     * @param  string|null $klarna_idempotency_key This header will guarantee the idempotency of the operation. The key should be unique and is recommended to be a UUID version 4. Retries of requests are safe to be applied in case of errors such as network errors, socket errors and timeouts. Input values of the operation are disregarded when evaluating the idempotency of the operation, only the key matters. (optional)
     * @param  string $contentType The value for the Content-Type header. Check self::contentTypes['refundOrder'] to see the possible values for this operation
     *
     * @throws \Klarna\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return array of string|\Klarna\OrderManagement\Model\RefundNotAllowedErrorMessage|\Klarna\OrderManagement\Model\NotFoundErrorMessage, HTTP status code, HTTP response headers (array of strings)
     */
    public function refundOrderWithHttpInfo($order_id, $refund_object, $klarna_idempotency_key = null, string $contentType = self::contentTypes['refundOrder'][0])
    {
        $request = $this->refundOrderRequest($order_id, $refund_object, $klarna_idempotency_key, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function refundOrderAsync($order_id, $refund_object, $klarna_idempotency_key = null, string $contentType = self::contentTypes['refundOrder'][0])
    {
        return $this->refundOrderAsyncWithHttpInfo($order_id, $refund_object, $klarna_idempotency_key, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function refundOrderAsyncWithHttpInfo($order_id, $refund_object, $klarna_idempotency_key = null, string $contentType = self::contentTypes['refundOrder'][0])
    {
        $returnType = 'string';
        $request = $this->refundOrderRequest($order_id, $refund_object, $klarna_idempotency_key, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function refundOrderRequest($order_id, $refund_object, $klarna_idempotency_key = null, string $contentType = self::contentTypes['refundOrder'][0])
    {
